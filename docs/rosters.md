...
| Case                                                                         | What happens                                                                                                                                                                                                                                  |
| ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Member holds two rank roles** (promotion where nobody removed the old one) | Highest `seniority` wins, deterministically. Ties break on rank id, so the answer never depends on row order.                                                                                                                                 |
| **Member is on two rosters in one guild**                                    | Both memberships are kept and published. The nickname is written by exactly one roster — lowest `position`, then slug — because a nickname has room for one rank. Without this the two rosters overwrite each other on every sync.            |
| **A rank role is deleted in Discord**                                        | The rank is unbound and its holders stay on the roster, unranked. Left bound, the next sweep would find nobody holding a role that no longer exists and strip the entire rank from the roster and their nicknames.                            |
| **A reconciliation would remove many members at once**                       | Paused, nothing applied, `THRESHOLD_EXCEEDED` issue raised and global admins alerted. Uses the same `sync.maxRemovalsThreshold` setting as role sync. A mass removal is nearly always a deleted or unbound rank role, not a mass resignation. |
| **Two members given the same callsign**                                      | Refused with a conflict naming who already holds it. Checked per roster, against active members only.                                                                                                                                         |
| **Roster has no ranks bound yet**                                            | Skipped entirely. Reconciling it would read as "nobody holds a rank" and empty it.                                                                                                                                                            |
| **Member leaves the Discord server**                                         | Marked `DEPARTED`, kept as history. No nickname to write.                                                                                                                                                                                     |
| **Member is re-hired**                                                       | The existing row is reactivated, keeping their callsign and any name override.                                                                                                                                                                |
| **Member edits their own nickname**                                          | The managed format is restored around whatever name they typed — unless an administrator pinned a name, which wins.                                                                                                                           |
| **A rank role is moved above the bot**                                       | The roster stays correct; the rename fails with `MEMBER_ABOVE_BOT` and is recorded, not retried blindly.                                                                                                                                      |
| **Redis is unreachable**                                                     | Nickname events are ignored rather than risking a rename loop. The scheduled sweep repairs the drift.                                                                                                                                         |
| **Two jobs touch one member at once**                                        | The per-member lock serialises them; the loser skips, because reconciliation is idempotent and the winner computes the same answer.                                                                                                           |
