/**
 * `/requesttraining` — file a training request in this guild's request channel.
 *
...
export const data = addRequestOptions(
  new SlashCommandBuilder().setName('requesttraining').setDescription('Request a training — opens a thread and pings the trainers').setDMPermission(false),
  'training',
);

export async function execute(interaction) {
  return runRequest(interaction, 'training');
}
